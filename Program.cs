...
// Wrong way: blocking with Thread.Sleep
for (int i = 0; i < 5; i++)
    Thread.Sleep(300);
...
// ============================================================
// Exercise 6 Step 2 & 3: Fetch Students and Courses in Parallel
// ============================================================
...
foreach (var fs in fetchedStudents)
    Console.WriteLine($"  {fs.Name} GPA: {fs.GPA}");
...
foreach (var fs in fetchedStudents)
{
    try
    {
        var record = enrollService.ProcessRegistration(fs, enrollCourse);
        enrollCourse.EnrolledCount++;
        enrollments.Add(record);
        Console.WriteLine($"  Enrolled: {fs.Name}");
    }
    catch (InvalidOperationException ex)
    {
        failures.Add($"{fs.Name}: {ex.Message}");
        Console.WriteLine($"  Rejected: {fs.Name} — {ex.Message}");
    }
}
